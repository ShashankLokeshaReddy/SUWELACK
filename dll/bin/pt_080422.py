...
def endta51cancelt905(apersnr):
	...
	print(f"[DLL] endta51cancel: xret: {xret}, xfa: {xfa}, xgk: {xgk}")
	if xret is None:
		xret=''
		...
		if kt002.gtv("TA22_Typ")  == "7":
			...
			if kt002.gtv("TA22_Dauer") != 1:
				...
				xret = kt002.BuchTA51_3(xTSEnd, int(kt002.gtv("T910_Nr")), kt002.gtv("TA06_FA_Nr"),
										...
										kt002.gtv("T905_Nr"),kt002.gtv("TA06_TE"),kt002.gtv("TA06_TR"),0.0,xmegut,0.0,
										xTRMan, xTA11Nr, xCharge,xVal1,xVal2,xVal3,xVal4,xVal5,kt002.gtv("TA06_FA_Art"),xTS)
	        #EvtMsgDisplay("FA Buchen", "MSG0166", kt002.dr_TA06("TA06_BelegNr"), kt002.dr_TA06("TA06_AgBez"))
...
def bufa(ANr,ATA29Nr,AFARueckend,ata22dauer):
	...
	if len(xFehler) == 0:
		...
		if len(xFehler) == 0:
			if xbBuchZiel == 1:
				print("execute fabuchta55")
				xFehler = fabuchta55()
			else:
				print("execute fabuchta51")
...
def actbuchung(ta29nr,AKst,ASA,t905nr,ASALast,AKstLast,ATSLast,APlatz):
  ...
  #ByRef ASALast As String, ByRef AKstLast As String, ByRef ATSLast As String, ByRef AT905Last As String, ByRef ATA29Last As String) as string
	print(f"[DLL] PRE CheckKommt: ASA: {ASA}, AKst: {AKst}, ASALast: {ASALast}, AKstLast: {AKstLast}, ATSLast: {ATSLast}, xT905Last: {xT905Last}, xTA29Last: {xTA29Last}")
	...
	print(f"[DLL] CheckKommt: xret: {xret}, ASALast: {ASALast}, ATSLast: {ATSLast}, xT905Last: {xT905Last}, xTA29Last: {xTA29Last}")
	print(result)
	if len(xret) > 0:
		#'!!!MSG Warnung !!!!
	...
	if kt002.CheckObject(kt002.dr_TA06) == True or kt002.CheckObject(kt002.dr_TA05) == True:
		...
		if len(xret) == 0:
			...
			print(f"returning xret: {xret}")
			return xret
		
	if len(xret) == 0:
		print(f"[DLL] PRE PNR_Buch: xret: {xret}, ASA: {ASA}, AKst: {AKst}, t905nr: {t905nr}, ta29nr: {ta29nr}, APlatz: {APlatz}, xtagid: {xtagid}, xkstk: {xkstk}")
		result = kt002.PNR_Buch(ASA, AKst, t905nr, ta29nr, APlatz, xtagid, xkstk)
		xret,ASA,AKst,APlatz,xtagid,xkstk=result
		print(f"[DLL] PNR_Buch: xret: {xret}, ASA: {ASA}, AKst: {AKst}, APlatz: {APlatz}, xtagid: {xtagid}, xkstk: {xkstk}")
		...
		if len(xret) == 0:
			...
			if len(xret) == 0:
				print(f"[DLL] PNR_Buch3: ta29nr: {ta29nr}, xfaruecknr: {xfaruecknr}, xmenge: {xmenge}")
...
def ta06gk(ABelegNr,AktAction,AScreen2,ATA29Nr,AKst,ASA,AFA):
	...
	if len(xBelegNr) > 0:
		if kt002.TA06Read(xBelegNr) == True:
			...
			print("Auftrag ohne Kommt 1")
	else:
		print("Auftrag ohne Kommt 2")
...
#Debugmode
BUCHFA = 1 #0= K/G 1=FA Buchen 2=WB
...
if BUCHFA == 1:
	belegnr ='GK0022400'  # GK002 ist GK nummer, 240 ist Arbeitsgang und 0 ist sort-nummer
	buaction=1
	xKst=''
	xsa=''
	xfa=''     
...
#Wechselbuchung - mit scanvalue Persnr
if BUCHFA == 2:
	belegnr =''
	platz='6001'
	t905nr='6001'
...
if BUCHFA == 3:
	belegnr ='110003230200017_6001_610460010'
	buaction=1
	xKst=''
	xsa=''
	xfa='110003230200017_6001_610460010'     
	print('BUFA = 3 Read Beleg')
	#kstf_TA06Belegnr muß bei Rotpunkt noch auf Read TA06_FAD1 eingestellt sein!!!!
	result = kt002.TA06Read(belegnr)
	print (result)
	ret=True 
...
if ret == True:
		print('Pruef_PNrFkt')
		print(f"[DLL] PRE PruefPNr bufunktion: {type(bufunktion)}, scantype: {type(scantype)}, sa: {type(sa)}, buaction: {type(buaction)}, appmscreen2: {type(appmscreen2)}, scanon: {type(scanon)}, activefkt: {type(activefkt)}, msg: {type(msg)}")
		print(f"[DLL] PRE PruefPNr bufunktion: {bufunktion}, scantype: {scantype}, sa: {sa}, buaction: {buaction}, appmscreen2: {appmscreen2}, scanon: {scanon}, activefkt: {activefkt}, msg: {msg}")
		result = kt002.Pruef_PNrFkt(bufunktion,scantype,sa,buaction, appmscreen2, scanon,activefkt, msg)
		ret,sa,buaction,activefkt,msg=result
		print('ret,sa,buaction,activefkt,msg')
		print(result)
