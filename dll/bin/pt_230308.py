...
def bufa(ANr,ATA29Nr,AFARueckend,ata22dauer):
	...
	if kt002.CheckObject(kt002.dr_TA06) == True:
		...
		if kt002.CheckObject(kt002.dr_TA05) == True:
			...
			print("[NEW] TA06ReadArbGrNr: ", xfanr, xt905nr)
	...
	if len(xFehler) == 0:
	    ...
	    #setzen Zieltabelle
		print("[NEW] BuFANr0Status: ", xbBuchZiel)
...
def actbuchung(ta29nr,AKst,ASA,t905nr,ASALast,AKstLast,ATSLast,APlatz):
  ...
  #ByRef ASALast As String, ByRef AKstLast As String, ByRef ATSLast As String, ByRef AT905Last As String, ByRef ATA29Last As String) as string
	print("[NEW] CheckKommt: ", sa, kst, salast, kstlast, tslast, xT905Last, xTA29Last)
		...
		if len(xret) == 0:
			if kt002.CheckObject(kt002.dr_T905) == False:
				...
				if kt002.CheckObject(kt002.dr_T951) == True:
					print("dr_T951 True")
					print("[NEW] T905Read: ", kt002.gtv("T951_Arbist"))
...
#in bufa   xbBuchZiel = 2
# damit Zähler rückmelden
def fabuchta56():
	xInputMenge=0 #Flag, 1=Menge eingeben
	xInputMengeNew=0
	xFARueckEnd=False
	...
	if kt002.CheckObject(kt002.dr_TA06) == True:
		#xbeleg = kt002.dr_TA06("TA06_BelegNr")
		xbeleg = kt002.gtv("TA06_BelegNr")
		
	print(f"Belegnr check:: {xbeleg}")
		
	result = kt002.BuchTA56_0(xPlatz) #letzten Zählerstand holen und Platz wird aus Soll_Platz des FA geholt (jeder Zähler ist einem anderen Platz zugeordnet, Person bucht für andere)
	xret,xPlatz=result
	
	if kt002.CheckObject(kt002.dr_TA56) == True:
		xIstMeGes = kt002.gtv("TA56_Wert") #letzte Zählerstand
	if kt002.CheckObject(kt002.dr_T910) == True:
		xPersNr = int(kt002.gtv("T910_Nr"))
	if kt002.CheckObject(kt002.dr_TA06) == True:
		xTE = kt002.gtv("TA06_TE")

  #Mengendialog
  #xbBuchen = kt001_InputMenge_Modus(Nothing, ActModus, kt002.dr_TA06("TA06_RueckArt"),
  # kt002.dr_TA06("TA06_BelegNr"), kt002.dr_TA06("TA06_AgBez"), xPlatz, 0, xIstMeGes, 0 _
  # , xMengeGut, xMengeAus, xTRMan, xTA11nr, xStatusMenge, xCharge, xVal1, xVal2, xVal3, xVal4, xVal5, 0, xClDetails, xDatumTS)

	#Debug-Werte
	xVal1 = 79200 #neuer Zählerstand
	xMengeGut = xVal1 - xIstMeGes # bspw. 45 Differenz neuer Zähler - Ges.Istmenge
	# wenn Differenz negativ, Fehlermeldung
	xMengeAus = 0
	xDatumTS = datetime.now() #aus Mengendialog
	xTS=xDatumTS.strftime("%d.%m.%Y %H:%M:%S")  #Stringtransporter Datum 
	
	
	if xbuchen == True:
		kt002.BuchTA56_3(xFAStatus, xTS, xPlatz, xPersNr, xMengeGut, xMengeAus, xTE, xtrman, xta11nr, xcharge, xVal1, xVal2, xVal3, xVal4, xVal5, xScanFA)
...
#print("------------------------------------------------------")
# time.sleep(3)
do_stuff(ascanvalue="FAZ030100")  # FA Buchen auf 0006  funktion: fabuchta56
print("------------------------------------------------------")
time.sleep(3)
do_stuff(ascanvalue="1000")  #  buchen
# print("------------------------------------------------------")
# time.sleep(3)
