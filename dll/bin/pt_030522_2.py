...
def fabuchta51(ata22dauer):
	...
	print(f"PRE BUCHTA51_0: xret:{xTA22Dauer}, xTS:{xTS}, xStatusMenge:{xStatusMenge}")
	result = kt002.BuchTA51_0(xTA22Dauer, xTS,  xStatusMenge)
	xret,xTS,xStatusMenge=result
	print(f"BUCHTA51_0: xret:{xret}, xTS:{xTS}, xStatusMenge:{xStatusMenge}")
		...
		if kt002.CheckObject(kt002.dr_TA06) == True:
			if kt002.gtv("T951_Arbist") != kt002.gtv("TA06_Platz_Soll"):
				...
				if T905AllowRoute == True:
					...
					if RouteDialog == 0:
						# Abweichender Umrouten Umbuchen?
...
def do_stuff(debug, change_workstation=False, gemeinkosten_buttons=False, gemeinkosten_beenden=False):
	checkfa = False
	aktscreen = 1  # 0-none,1-dgt001,2-dgt002,3-dgt800
	bufunktion = 6  # Buchungsfunktion
	showhost = 1  # Anzeige Hostinformation im Terminal
	...
	if change_workstation:
		kt002.T905Read('F004')

	#Debugmode
	BUCHFA = 99 #0= K/G 1=FA Buchen 2=WB

	# scanvalue="1035" #gescannter Wert
	scanvalue=debug #gescannter Wert
	...
	#            , ByVal ASerial As Boolean, ByRef AActiveFkt1 As String, ByRef AMsg As String, ByRef AMsgFkt As String, ByRef AMsgDlg As String) As Boolean
			...
			print('Vor Pruef_PNRFkt Werte: bufunktion,scantype,sa,buaction,appmscreen2,scanon,activefkt,msg')
			print(str(bufunktion) + ":" + str(bufunktion) + ":"+  str(scantype) +":"+   str(sa) + ":"+  str(buaction) +":"+   str(appmscreen2) +":"+ str(scanon) +":"+   str(activefkt) +":"+   str(msg))
			msgfkt=""
			msgdlg=""
			serial=True

			if gemeinkosten_buttons:
				activefkt = "TA06GK"

			if gemeinkosten_beenden:
				activefkt = "TA06END"
			...
			print('Nach Pruef_PNRFkt Werte: ret,sa,buaction,activefkt,msg')
			print(result)
			...
			#TA06GK - GK beginnen
			if buaction == 9:  # Gemeinkostenbutton dialog anzeigen
				belegnr = ""
				xKst = ""
				xsa = ""
				xfa = ""
...
do_stuff(debug="1024")  # Kommen
print("------------------------------------------------------")
time.sleep(3)
do_stuff(debug="1024", change_workstation=True)  # Arbeitsplatzwechsel
...
# GK Buttons beginnen
# time.sleep(2)
# do_stuff(debug="1024", gemeinkosten_buttons=True)
# print("------------------------------------------------------")
# ofenen GK beenden
# time.sleep(5)
# do_stuff(debug="1024", gemeinkosten_beenden=True)

"""
print("------------------------------------------------------")
# GK beginnen (GK Nummer über Eingabefeld)
time.sleep(2)
do_stuff(debug="GK0120350")
print("")
time.sleep(2)
do_stuff(debug="1024")
print("------------------------------------------------------")
