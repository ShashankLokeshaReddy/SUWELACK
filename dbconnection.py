...
def getArbeitplazlist(connection):
    arbeitplatzlist = pd.read_sql_query("SELECT T905_Nr,T905_bez,T905_Bez2,T905_KstNr,T905_Art,T905_Typ,T905_Akkord,T905_Leist,T905_Freigabe,T905_ArbGrNr,T905_Img,T905_StartScreen,T905_Schleuse "
                                   "FROM ksalias.dbo.G905_TermPlatz INNER JOIN ksalias.dbo.T905_ArbMasch ON G905_Platz = T905_nr AND G905_FirmaNr = T905_FirmaNr "
...
def getPersonaldetails(connection):
    personalname = pd.read_sql_query("SELECT T912_Nr,T910_Name,T910_Vorname FROM ksalias.dbo.T910_Personalliste "
                                     "INNER JOIN ksalias.dbo.T912_PersCard ON T910_Nr = T912_PersNr", connection, coerce_float=False)
